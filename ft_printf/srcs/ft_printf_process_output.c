...
/*   Updated: 2022/04/26 14:21:48 by kshim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	len_of_output(t_fp_formats *formats, t_fp_str *data)
{
	...
	if (fs_process_ap(formats, data) == -1)
		return (-1);
	return (1);
}
...
char	*put_chars(t_fp_formats *formats, t_fp_str *data)
{
	...
	i = 0;
	j = 0;
	if (formats -> left_justify == 1 || formats -> zero_fill == 1)
	{
	...
	if (formats -> left_justify == 0 && formats -> zero_fill == 0)
	{
		if (formats -> plus_sign == 1)
			str[i++] = '+';
		else if (formats -> space_sign == 1)
